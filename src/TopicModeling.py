...
def topic_modeling(num_topics=20, filterExtremes=False, library='gensim', logger= None):
    logger.critical(
        "num_topics=" + str(num_topics) + " filterExtremes=" + str(filterExtremes) + " library=" + str(library)+'\n')
    processed_docs, documents = dp(userModeling=True, timeModeling=True, preProcessing=False, TagME=False, logger=logger)
    # logger.critical("\nReturn to TopicModeling.py\n")
    pp = np.asarray(processed_docs)
    # print(processed_docs)
    print('processed_docs shape')
    print(pp.shape)
    logger.critical("Shape of processed_docs (the output of DataPreparation file):"+ str(pp.shape)+'\n')
    # print(pp)
    ...
    try:
        ...
        logger.critical('Coherence value is:'+str(c)+'\n')
        logger.critical('Topic coherences are:' + str(cv)+'\n')
    except:
        pass
