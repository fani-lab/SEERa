...
def main(user_clusters, user_final_interests, news_table):
    ...
    community_recommendations_main = pd.DataFrame([community_recommendations.columns[row.argsort()[-params.apl['topK']:][::-1]] for row in similarities],columns=[f'TopNews_{i + 1}' for i in range(min(len(similarities[-1]),params.apl['topK']))]).join(community_recommendations[['Community']])
    ...
    user_recommendations_main = pd.DataFrame([user_recommendations.columns[row.argsort()[-params.apl['topK']:][::-1]] for row in user_news_similarities],columns=[f'TopNews_{i + 1}' for i in range(min(len(similarities[-1]),params.apl['topK']))]).join(user_recommendations[['UserId']])
